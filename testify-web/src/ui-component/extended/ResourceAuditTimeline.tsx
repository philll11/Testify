...
import { Box, Stack, Typography, Chip, Accordion, AccordionSummary, AccordionDetails } from '@mui/material';
import { IconHistory, IconPencil, IconPlus, IconTrash, IconRotate, IconChevronDown } from '@tabler/icons-react';
...
const getActionIcon = (action: AuditAction) => {
  switch (action) {
    case AuditAction.CREATE:
      return <IconPlus size="1.1rem" />;
    case AuditAction.UPDATE:
      return <IconPencil size="1.1rem" />;
    case AuditAction.DELETE:
      return <IconTrash size="1.1rem" />;
    case AuditAction.REOPEN:
      return <IconRotate size="1.1rem" />;
    default:
      return <IconHistory size="1.1rem" />;
  }
};
...
const ResourceAuditTimeline = ({ resource, resourceId }: ResourceAuditTimelineProps) => {
  ...
  if (error)
    return (
      <Box p={3}>
        <Typography color="error">Failed to load audit history.</Typography>
      </Box>
    );
  ...
  return (
    <Box sx={{ maxWidth: '100%', overflowX: 'hidden' }}>
      {history.map((entry, index) => (
        <AuditItem key={entry.id || index} entry={entry} isLast={index === history.length - 1} />
      ))}
    </Box>
  );
};
