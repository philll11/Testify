...
describe('Roles Auth (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        viewRoleToken = await createRoleAndUser('ROLE_VIEWER', 'Role Viewer', [PERMISSIONS.ROLE_VIEW], 'VIEW');
        editRoleToken = await createRoleAndUser('ROLE_EDITOR', 'Role Editor', [PERMISSIONS.ROLE_EDIT], 'EDIT');
        createRoleToken = await createRoleAndUser('ROLE_CREATOR', 'Role Creator', [PERMISSIONS.ROLE_CREATE], 'CREATE');
        deleteRoleToken = await createRoleAndUser('ROLE_DELETER', 'Role Deleter', [PERMISSIONS.ROLE_DELETE], 'DELETE');
        noPermissionsToken = await createRoleAndUser('ROLE_NO_PERMISSIONS', 'No Permissions', [], 'NOPERMS');
        manageInactiveToken = await createRoleAndUser('ROLE_INACTIVE_MANAGER', 'Inactive Manager', [PERMISSIONS.ROLE_VIEW, PERMISSIONS.ROLE_EDIT, PERMISSIONS.ROLE_MANAGE_INACTIVE], 'INACTIVE');
    });
