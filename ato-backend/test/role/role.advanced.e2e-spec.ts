...
describe('Roles Advanced (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    roleInUse = await roleRepository.save(
      roleRepository.create({
    ...
    userWithRole = await userRepository.save(
      userRepository.create({
  ...
  describe('DELETE /roles/:id', () => {
    it('should return 409 when trying to delete a role that is in use', async () => {
      ...
      expect(response.body.message).toContain(
        'Cannot delete Role. Assigned to one or more Users.',
      );
    });
  });
  ...
  describe('PATCH /roles/:id', () => {
    it('should return 409 when trying to deactivate a role that is in use', async () => {
      ...
      expect(response.body.message).toContain(
        'This role cannot be deactivated because it has 1 active user(s) assigned to it.',
      );
    });
  });
});
