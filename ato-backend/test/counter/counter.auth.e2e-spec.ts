...
describe('Counters Auth (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        // Helper
        const createPersona = async (roleRecordId: string, roleName: string, perms: string[], userRecordId: string, email: string) => {
        ...
        adminToken = await createPersona(
            'ROLE_C_AUTH_ADMIN', 'Counter Admin',
            [PERMISSIONS.COUNTERS_VIEW, PERMISSIONS.COUNTERS_EDIT],
            'USER_C_AUTH_ADMIN', 'counter.auth.admin@test.com'
        );

        // 2. View Only
        viewOnlyToken = await createPersona(
            'ROLE_C_AUTH_VIEW', 'Counter Viewer',
            [PERMISSIONS.COUNTERS_VIEW],
            'USER_C_AUTH_VIEW', 'counter.auth.view@test.com'
        );

        // 3. No Perms
        noPermsToken = await createPersona(
            'ROLE_C_AUTH_NONE', 'No Counter Perms',
            [], // No permissions related to counters
            'USER_C_AUTH_NONE', 'counter.auth.none@test.com'
        );
