...
describe('Users Auth (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        const createRoleAndUser = async (recordId: string, name: string, permissions: string[], userPrefix: string) => {
            // Check if exists
            let role = await roleRepository.findOne({ where: { recordId } });
            if (!role) {
                role = await roleRepository.save(roleRepository.create({
                    recordId, name, permissions
                }));
            }

            let user = await userRepository.findOne({ where: { recordId: `${userPrefix}_USER` } });
            if (!user) {
        ...
        viewUserToken = await createRoleAndUser('ROLE_VIEWER_USER', 'User Viewer', [PERMISSIONS.USER_VIEW], 'VIEW_USER');
        editUserToken = await createRoleAndUser('ROLE_EDITOR_USER', 'User Editor', [PERMISSIONS.USER_EDIT], 'EDIT_USER');
        createUserToken = await createRoleAndUser('ROLE_CREATOR_USER', 'User Creator', [PERMISSIONS.USER_CREATE], 'CREATE_USER');
        deleteUserToken = await createRoleAndUser('ROLE_DELETER_USER', 'User Deleter', [PERMISSIONS.USER_DELETE], 'DELETE_USER');
        manageUserInactiveToken = await createRoleAndUser('ROLE_INACTIVE_MGR_USER', 'User Inactive Manager', [PERMISSIONS.USER_VIEW, PERMISSIONS.USER_EDIT, PERMISSIONS.USER_MANAGE_INACTIVE], 'INACT_USER'); // Needs edit to reach isActive check
        noPermissionsToken = await createRoleAndUser('ROLE_NO_PERMS_USER', 'No Perms User', [], 'NO_PERMS_USER');
    });
    ...
    beforeEach(async () => {
        ...
        let role = await roleRepository.findOne({ where: { recordId: 'TEST_ROLE_USER_AUTH' } });
        if (!role) {
            role = await roleRepository.save(roleRepository.create({
        ...
        let user = await userRepository.findOne({ where: { recordId: 'TEST_USER_AUTH' } });
        if (user) {
