...
describe('Users Advanced Logic (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    standardRole = await roleRepository.save(
      roleRepository.create({
  ...
  beforeEach(async () => {
    ...
    const existing = await userRepository.findOne({
      where: { recordId: 'STD_USR_ADV' },
    });
    if (existing) await userRepository.remove(existing);

    standardUser = await userRepository.save(
      userRepository.create({
    ...
    standardUserToken = jwtService.sign({
      sub: standardUser.recordId,
      tokenVersion: 0,
    });
  });
