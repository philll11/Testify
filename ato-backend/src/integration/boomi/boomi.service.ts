...
export class BoomiService implements IIntegrationPlatformService {
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (axios.isAxiosError(error) && error.response) {
                    ...
                    if (status === 503 || status === 504 || status === 429) { // Rate Limit Exceeded or Gateway Timeout
        ...
        try {
            ...
            if (axios.isAxiosError(error) && error.response) {
                ...
                if ([401, 403].includes(status)) {
                    throw new UnauthorizedException('Boomi API authentication failed. Please check your credentials.');
                }
            ...
            throw new IntegrationPlatformException(`Failed to fetch metadata for component ${componentId}: ${message}`);
        }
    }
    ...
    public async getComponentInfoAndDependencies(componentId: string): Promise<ComponentInfo | null> {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : 'An unknown error occurred';
            throw new IntegrationPlatformException(`Failed to fetch dependencies for component ${componentId}: ${message}`);
        }
    }
