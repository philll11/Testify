...
export default function Alert(theme: any) {
  ...
  const getPaletteColor = (severity: any) => (severity ? vars.palette[severity] : vars.palette.info);

  const getCommonStyles = (ownerState: any) => {
    const isWarningOrSuccess = ownerState.severity === 'warning' || ownerState.severity === 'success';
    return { isWarningOrSuccess };
  };

  const standardVariant = ({ ownerState }: any) => {
  ...
  const outlinedVariant = ({ ownerState }: any) => {
    const paletteColor = getPaletteColor(ownerState.severity);
    const { isWarningOrSuccess } = getCommonStyles(ownerState);
  ...
  const filledVariant = ({ ownerState }: any) => {
    const paletteColor = getPaletteColor(ownerState.severity);
    const { isWarningOrSuccess } = getCommonStyles(ownerState);
