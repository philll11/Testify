...
import {
  usePlatformProfiles,
  useCreatePlatformProfile,
  useUpdatePlatformProfile,
  useDeletePlatformProfile
} from 'hooks/platform/useProfiles';
...
const ProfileList = () => {
  ...
  const handleFormValuesChange = useCallback(
    (values: Partial<PlatformProfileFormData>) => {
      if (mode === 'create') {
            ...
            <>
              {can(PERMISSIONS.PLATFORM_PROFILE_VIEW) && (
                <Tooltip title="View Details">
                  <IconButton color="primary" size="small" onClick={(e) => handleViewPage(profile.id, e)}>
                    <IconEye size={18} />
                  </IconButton>
                </Tooltip>
              )}
              {can(PERMISSIONS.PLATFORM_PROFILE_EDIT) && (
                <>
                  <Tooltip title="Edit">
                    <IconButton color="secondary" size="small" onClick={(e) => handleEditPage(profile.id, e)}>
                      <IconEdit size={18} />
                    </IconButton>
                  </Tooltip>
              ...
              {can(PERMISSIONS.PLATFORM_PROFILE_DELETE) && (
                <Tooltip title="Delete">
                  <IconButton color="error" size="small" onClick={(e) => handleDeleteClick(profile, e)}>
                    <IconTrash size={18} />
                  </IconButton>
                </Tooltip>
              )}
          ...
          <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {mode === 'view' && selectedProfile && (
              <Stack direction="row" spacing={1}>
                ...
                {can(PERMISSIONS.PLATFORM_PROFILE_DELETE) && (
                  <Tooltip title="Delete">
                    <IconButton size="small" onClick={(e) => handleDeleteClick(selectedProfile, e)} color="error">
                      <IconTrash size={18} />
                    </IconButton>
                  </Tooltip>
                )}
