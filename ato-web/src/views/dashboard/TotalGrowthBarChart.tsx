...
export default function TotalGrowthBarChart({ isLoading }: TotalGrowthBarChartProps) {
  ...
  const grey500 = (theme.vars || theme).palette.grey[500];

  const primary200 = (theme.vars || theme).palette.primary[200];
  const primaryDark = (theme.vars || theme).palette.primary.dark;
  const secondaryMain = (theme.vars || theme).palette.secondary.main;
  const secondaryLight = (theme.vars || theme).palette.secondary.light;
  ...
  useEffect(() => {
    setChartOptions({
      ...
      yaxis: { ...(barChartOptions as any).yaxis, labels: { style: { colors: textPrimary } } },
